...
const GITHUB_TOKEN = process.env.GITHUB_TOKEN; // Optional for auto-commit

if (!OMDB_API_KEY) {
    console.error('OMDB_API_KEY environment variable is required');
    process.exit(1);
}
...
async function commitToGitHub(movieName, slug) {
    if (!GITHUB_TOKEN) {
        console.log('No GITHUB_TOKEN provided, skipping GitHub commit');
        return;
    }

    try {
        // Configure git
        execSync('git config --local user.email "admin@cardcraftmovies.com"');
        execSync('git config --local user.name "CardCraft Admin"');
        ...
        // Commit
        execSync(`git commit -m "Add movie: ${movieName} (auto-generated)"`);

        // Push
        execSync(`git push https://${GITHUB_TOKEN}@github.com/YOUR_USERNAME/YOUR_REPO.git main`);
...
app.post('/generate', async (req, res) => {
    try {
        ...
        console.log(`Generating movie page for: "${name}"`);

        // Fetch movie data from OMDb API
        const apiUrl = `https://www.omdbapi.com/?t=${encodeURIComponent(name)}&apikey=${OMDB_API_KEY}`;
        ...
        console.log(`Generated movie page: movies/${slug}.html`);

        // Optional GitHub commit
        await commitToGitHub(name, slug);

        // Return success response
        res.json({
            success: true,
            message: `Movie "${movieData.Title}" generated successfully`,
            slug: slug,
            html: htmlContent // For manual download if no GitHub commit
        });
...
// Start server
app.listen(PORT, () => {
    console.log(`CardCraft Movies Admin Generator running on port ${PORT}`);
});
